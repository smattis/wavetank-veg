...
opts=Context.Options([
    ...
    ("veg_denisty", 400, "stems per square meter (200 or 400)"),
...
def waveHeight(x,t):
    # import pdb
    # pdb.set_trace()
    return  float(interp_phi.__call__(t%interp_time[-1])) 
...
def waveHeight(x,t):
    return  float(interp_phi.__call__(t%interp_time[-1])) 
def waveVelocity_u(x,t):
    return interpU.__call__(t%interp_time[-1],x[1])[0][0] 
def waveVelocity_w(x,t):
    return 0.0
def waveVelocity_v(x,t):
    return interpW.__call__(t%interp_time[-1],x[1])[0][0]
